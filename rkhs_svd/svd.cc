// $Id: svd.cc,v 3.0 2004/11/19 18:58:36 foster Exp $
...
main(int argc, char** argv)
{
  ...
  int number_written = print_covariance(p_raw_data,covariance("raw principle component"),.99,.001); 
  ...
  number_written = print_covariance(p_centered_data,covariance("centered principle component"),.99,.001);
  ...
  int base_10 = 0;
  while(((number_created > 2) || ratio < 10000) && target > 0)
    {
...
print_covariance(gsl_matrix* p_raw_data, const kernel& k,double ratio_spread, double smallest)
{
  ...
  while((current_index < p_S->size) &&	(current_eigen > smallest * max_eigen))
	...
	{
	  ++result;
	  std::cout << k.name() << "@" << round_to_2_digits(current_eigen/max_eigen) << std::endl;
	  if(!use_sampled_eigen)
