...
setwd( path <- "~/C/text/prep_error/auction_run/"  )
...
Data <- read.delim("model_data.csv", header=T); names(Data)
...
tapply(Data$Residual, as.factor(Data$Role), mean)
tapply(Data$Residual, as.factor(Data$Role), sd)
