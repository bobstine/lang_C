...
\item[Custom experts] are spawned when a feature is added to the model.  By
 ...
 time lagsa segmentation of itself (to allow spatial heterogeneity, for
 ...
 experts are funded from the payout received when a variable joins the model.
 ...
 payout $\omega$.  Some fraction of this payout goes directly back to the
 recommending expert $\cal F$, the rest of the payout goes to experts related to
 ...
 the payout on $X$ when $X$ is accepted.\footnote{The code that builds custom
...
\end{description}
 ...
 experts using ``taxes'' in order to avoid adding further wealth beyound the
