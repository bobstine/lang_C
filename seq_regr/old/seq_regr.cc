/* $Id: seq_regr.cc,v 1.1 2007/11/21 23:18:27 bob Exp $
...
{
  int q (number_of_predictors());
  const double* ptr(mRegr.ols_beta());
  return make_anonymous_range(make_range(ptr, ptr+q));  // does not have the intercept
}
...
SequentialRegression::gaussian_predictor_stats (SequentialRegression::Vector z, const double zBar)
{
  ...
  */
  mRegr.evaluate_predictor(begin(z));
  return mRegr.f_test_evaluation();
...
SequentialRegression::bennett_predictor_stats (SequentialRegression::Vector z, double zBar)
{
  // test if z == mLastZ ???
  mRegr.evaluate_predictor(begin(z));
  return mRegr.Bennett_evaluation();
}
