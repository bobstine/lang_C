// $Id: seq_regr.h,v 1.1 2007/11/21 23:18:27 bob Exp $

/*
   5 Dec 05 ... Add selection weights.
...
const int MAX_REGR_MODEL_SIZE(1023);
...
class SequentialRegression
{
 ...
 private:
  int                     mN;                    // number of cases
  Vector                  mY;                    // response
  std::vector<int>        mSelection;            // inclusion weights for CV
 ...
 public:
  SequentialRegression (Vector y)
    :
    mN(end(y)-begin(y)), mY(y), mSelection(), mX(), mTags(), mIndex(), mZ(make_anonymous_range(y)),
