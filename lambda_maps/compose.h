...
/*********** compose_f_gx_t and compose_f_gx ***************  RAS  *******/

/* class for the compose_f_gx_y adapter, with f implicit
 */
template <class OP1, class OP2, class T>
class compose_f_gx_y_t : public std::unary_function<typename OP2::argument_type,
                                                    typename OP1::result_type>
{
 private:
  OP1 op1;
  OP2 op2;  // process: op1(op2(x),y)
  T mY;
 public:
  // constructor
  compose_f_gx_y_t(const OP1& o1, const OP2& o2, const T& y)
    : op1(o1), op2(o2), mY(y) { }
...
/* convenience functions for the compose_f_gx_y adapter, with minus implicit
 */
template <class OP1, class OP2, class T>
inline compose_f_gx_y_t<OP1,OP2,T>
compose_f_gx_y (const OP1& o1, const OP2& o2, const T& y) {
  return compose_f_gx_y_t<OP1,OP2,T>(o1,o2,y);
}
