// $Id: regression.cc,v 1.1 2005/06/13 20:47:51 bob Exp $

/*
   26 Feb 02 ... Add bennett code.
...
/*
  ...
  that member as well as the others which are not already present.
*/
int
...
{	
  ...
  count = nToAdd;
  if (UpdateCache(sm, pCols, nToAdd, 1) < 0) return -1;
  while (count > 0)
...
{	
  ...
  { // this is for hard threshold
    ...
    */
    // This version does the bennett rss sort, comparing a sequence of p-values
    /*
      Key_sorter::s_threshold =
...
void WriteCurrentFitSummary (SweepMatrix sm)
{
  ...
  BinomialVector(sm, &sumWts);
  ExitVoidOnNull(logFile = fopen (MakeFileName(sm, "regr.log"), "a"),
		 "could not open log file");
  fprintf (logFile,
	   "------------------------------------------------------------------\n");
  fprintf (logFile, "Sum of squared binomial wts = %15.9f\n",sumWts);
  fprintf (logFile,
...
//  Variations on fit properties:
//    s2   = (sse - *dRSS)/(df-1); // y'y-b'x'xb for model with added var
//    *fStat = (rcp * rcp) / rwwr; // s2 already part of rwwr (s2 * rwwr);
//    double beta_low = bennett_lower_bound(beta, a, b, cc);
//    *dRSS = beta_low * beta_low * rss;


/* This version returns properties to sort on change in RSS */
/*
  static void
  FitProperties (double rcp, double rss, double a, double b, double cc,
  double *dRSS, double *pVal)
  {   
  *dRSS = (rcp * rcp)/rss;
  double beta = ABS(rcp/rss);
  *pVal = bennett_p_value(beta, a, b);
  }
*/

/* This version finds lower bound for change to fit. */
static void
FitProperties (double rcp, double rss, double a, double b, double cc,
	       double *dRSS, double *pVal)
{   
...
FindNextVariables (SweepMatrix sm, Sort_tree *p_tree)
{
  ...
  Vector RSS, RCP; // diagonal of rWr and first row rWy 
  double sse, df, sumWts;
  long nCov;
  int *skip, i;
  char ch;
  FILE *logFile;
  ...
  // Track the max of the X values of potential predictors
  Vector a = NewVector(VectorLength(sm->residualSS),"FindNext");
  Vector b2 = NewVector(VectorLength(a), "FindNext");
  
  // Compute squared term for F-stat calculations
  // This version does the squared residual wts, the White estimator
  //   Vector wts = ResidualVector(sm);
  //   SquareVector(wts);
  // This version does the binomial wts
  Vector wts = BinomialVector(sm, &sumWts);
  Vector fit = FitVector(sm);
  FillResidualSSBDF(sm->bdf, NumberOfPredictors(sm), sm->fullSweepMat, skip, fit, wts, a, b2);
  CompactPrintVector(a,  "max absolute residual predictor");
  CompactPrintVector(b2, "b2 vector for p-value calculation");
  ...
  // Finish calculations for a and b2 by dividing by SSx
  for (int k=1; k<VectorLength(a); ++k) {
    if (!skip[k]) {
      double rss_k = VectorElement(RSS,k);
      DivideVectorElement(a ,k, rss_k);
      DivideVectorElement(b2,k, rss_k*rss_k);
    }
  }
  // Locate the columns offering most improvement (skip first since its Y)
  // by using the guaranteed RSS
  // Find first potential non-constant item for cache
  i = 0;  // 0 is Y
  while (++i < nCov) {
    double pval, dRSS;
    if (! skip[i]) {
      FitProperties(VectorElement(RCP,i),VectorElement(RSS,i),
		    VectorElement(a,i), sqrt(VectorElement(b2,i)),Key_sorter::s_threshold,
		    &dRSS, &pval);
      tree.insert(make_pair(Key(dRSS,pval), i));
    }
  }
  ExitVoidOnNull(logFile = fopen (MakeFileName(sm, "regr.log"), "a"), "could not open log file");
  fprintf (logFile, " ----  ([col], dRSS, p*pVal) sorted on dRSS if signif (q/p = %9.6f, qBase %d) -----\n",
	            Key_sorter::s_threshold, sm->qBase);
  Sort_tree::iterator treeIt;
  char sym;   int p = VectorLength(a)-1; 
  for(i=0, treeIt=tree.begin(), ch='+', sym=' '; i<10; ch=' ', ++treeIt, ++i)
  {
    Key sortKey = treeIt->first;    // <dRSS,pval> pair
    long k      = treeIt->second;
    double beta = VectorElement(RCP,k)/VectorElement(RSS,k);  beta = ABS(beta);
    double a_k  = VectorElement(a,k);
    double b_k  = sqrt(VectorElement(b2,k));
    double lb_k = bennett_lower_bound(beta,a_k,b_k,Key_sorter::s_threshold);
    if (sortKey.second <= Key_sorter::s_threshold)
      sym='S';  else sym='n';
    fprintf(logFile, "%c  %6ld %10.5f  %c p=%8.5f beta=%8.4f t=%6.2f  m=%6.3f lb=%7.4f (%g,%g,%g)\n",
	    ch, k, sortKey.first, sym, p*sortKey.second, beta, beta/b_k, a_k/b_k,
	    VectorElement(RSS,k) * lb_k * lb_k,   	    a_k, b_k, VectorElement(RSS,k));

  }
  fprintf (logFile, " ---------------------------------------------------------\n");
  DeleteVector(fit); DeleteVector(wts); DeleteVector(a); DeleteVector(b2);
  fclose(logFile);
}
...
#define MAX_PROB 0.99999
#define MIN_PROB 0.00001

Vector  BinomialVector (SweepMatrix sm, double *sumWts)
{
  Vector fitVector;
  double fit, fitVar;
  long nBig, nSmall, i;
  ...
  *sumWts = 0.0;
  ...
  for (i=0, nBig=0, nSmall=0; i<fitVector->len; ++i)
    {
      fit = VectorElement(fitVector,i);
      if (fit > MAX_PROB)
	{       fit = MAX_PROB;       ++nBig; }
      else if (fit < MIN_PROB)
	{       fit = MIN_PROB;       ++nSmall; }
      fitVar = fit * (1.0 - fit) ;    // p(1-p)
      AssignVectorElement (fitVector, i, fitVar);
      *sumWts = *sumWts + fitVar;
    }
  if (nBig || nSmall)
    fprintf(stdout,
	    "SM: #(predictions > %f) = %ld; #(preds < %f) = %ld\n", MAX_PROB,nBig, MIN_PROB,nSmall);
  return(fitVector);    // holds the variances on exit; over-written
}
...
CheckWeightVector(SweepMatrix sm, double *maxDiff, double *avgDiff, double *corr)
{
  ...
  newWts = BinomialVector (sm, &sumWts);
...
BinCovMatOfEstimates (SweepMatrix sm)
{
  ...
  wtVector = BinomialVector(sm, &sumWts);
