...
*     File Name: scopy.c                                                       *
*                                                                              *
*     This sample illustrates the usage of 'cblas_scopy()' function and do     *
...
void Dummy_cblas_scopy ( int N, const float* X, int incX, const float* Y, int incY );
...
void RunSCOPYSample ( void ) {
    ...
    // testing 'cblas_scopy()'
    printf("before copy contents in X.\n");
    for ( i = 0; i < N; i++ ){
        printf("%4.4f ", X[i]);
        }
    printf("\n");
    
    printf("\nbefore copy contents in Y.\n");
    for ( i = 0; i < N; i++ ){
        printf("%4.4f ", Y[i]);
        }
    printf("\n");
    ...
    printf("\nafter copy contents in X.\n");
    for ( i = 0; i < N; i++ ){
        printf("%4.4f ", X[i]);
        }
    printf("\n");
    
    printf("\nafter copy contents in Y.\n");
    for ( i = 0; i < N; i++ ){
        printf("%4.4f ", Y[i]);
        }
    printf("\n");

     // Timing section for 'cblas_scopy()'
      	{
            ...
            // Measure and take off the calling overhead of 'cblas_scopy()'        
            StartClock();
            for ( i = 0; i < MAX_LOOP_NUM; i++ )
            	{
                Dummy_cblas_scopy ( N, X, 1, Y, 1 );
                }
            ...
            printf ( "\nTime for performing 'cblas_scopy()' for 1000000 times is %4.4f µsecs.", time );
