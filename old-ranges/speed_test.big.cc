// $Id: speed_test.big.cc,v 1.1 2004/03/21 23:04:57 bob Exp $
...
public:
  ...
  {
    double ss  = range_ops::accumulate(make_unary_range(CenterSquare(xBar), make_range(x)),0.0);
    return ss / length;
  }
};
...
time_calculation_of_s2(const F& f, char *label, RandomGenerator& rand)
{
  ...
  for (int series = 0; series < 3; ++ series)
	...
	//	   << double(clock() - start)/CLK_TCK << std::endl;
...
class RangeIP {
  ...
  double operator()(const std::vector<double>& x,
		    ...
		    const std::vector<double>& w) const
    {
      ...
      // faster than iterator: 
      return range_ops::simple_accumulate(make_range(y) * make_range(x) * make_range(w), 0.0);
    }
};
...
class RangeProdIP {
  ...
  double operator()(const std::vector<double>& x,
    ...
    {
      using namespace apl;
      return range_ops::accumulate (make_range(y) * make_range(x) * make_range(w), 0.0);
    }
};
  ...
  for (int series = 0; series < 3; ++ series)
      ...
      for (int j=0; j < nReps; ++j)
	{
      ...
      std::cout << f.class_name() << ":" << label << ": Time for length " << length << " was "
	   << double(clock() - start)/CLOCKS_PER_SEC/10000 << " giving avg " << ip/nReps << std::endl;
...
class AccumIP2 {
  ...
  double operator()(const std::vector<double>& x,
		    const std::vector<double>& y) const
    {
      ...
      return range_ops::simple_accumulate (make_range(y) * make_range(x), 0.0);
    }
};
