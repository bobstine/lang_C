// $Id: baby_packed_sparse_iterator.cc,v 1.1 2003/11/26 14:11:50 bob Exp $
...
{
  typedef std::vector<std::pair<int, double> > map_type;
...
public:
  ...
  find_key(key_type i)
  {
    ...
    const_iterator result =  recursive_find_key(i,m_map.begin(),m_map.end());
    return result;
  }

  const_iterator
  recursive_find_key(key_type i,const_iterator lower, const_iterator upper)
  {
    if(upper - lower <= 1)
      return lower;
