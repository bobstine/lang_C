// $Id: anonymous_iterator.test.cc,v 1.20 2004/04/29 16:54:20 bob Exp $    
...
int main()
{
  ...
  anonymous_iterator_envelope<std::random_access_iterator_tag,double> c = make_anonymous_iterator(iz.begin());
  ...
  range<  anonymous_iterator_envelope<std::random_access_iterator_tag,double> > aRange = make_anonymous_range(iz);
  std::cout << "TEST: A range is " << aRange << std::endl;
  std::cout << "TEST: Sum of this of anonymous range is "
	    << range_ops::accumulate(make_anonymous_range(iz.begin(),iz.end()),0.0) << std::endl;
  std::cout << "shifted: "
	    << make_unary_range(Shifter(3.0),make_anonymous_range(iz.begin(),iz.end())) << std::endl;
