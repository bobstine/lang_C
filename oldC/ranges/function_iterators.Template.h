// $Id: function_iterators.Template.h,v 1.13 2004/03/16 18:19:30 bob Exp $ -*- c++ -*-
  ...
  : public extensible_unary_iterator<Host,I,std::bidirectional_iterator_tag>
{
 public:
  ...
  extensible_unary_iterator(const I& it)
    : extensible_unary_iterator<Host,I,std::bidirectional_iterator_tag>(it) { }
  ...
  Host&
  operator+=(int n) { mIter += n; return static_cast<Host&>(*this);}
  ...
  bool
    operator<(const type_of_this& it) const { return mIter < it.mIter; }
  bool
    operator>(const type_of_this& it) const { return mIter > it.mIter; }
  bool
    operator<=(const type_of_this& it) const { return mIter <= it.mIter; }
...
public:
  ...
  typename F::result_type
  operator*() const
  { return operator()(value()); }                        // apply F's operator() to underlying ex_iter's value
...
public:
  ...
  result_type
  operator*()
  { return value()(mX); }                             // apply iterator's value to slot value
  ...
  : public extensible_binary_iterator<Host,I1,I2,std::bidirectional_iterator_tag>
{
 public:
  ...
  extensible_binary_iterator(const I1& it1, const I2& it2)
    : extensible_binary_iterator<Host,I1,I2,std::bidirectional_iterator_tag>(it1,it2) { }
  ...
  Host&
  operator+=(int n) { mIter1 += n; mIter2 += n; return static_cast<Host&>(*this);}
 
  Host&
  operator-=(int n) { mIter1 -= n; mIter2 -= n; return static_cast<Host&>(*this);}

  typename std::iterator_traits<I1>::difference_type
    operator-(const type_of_this& it) const { return mIter2 - it.mIter2; }  // use second for 2 * X ops

  bool
    operator<(const type_of_this& it) const { return mIter2 < it.mIter2; } // second
  bool
    operator>(const type_of_this& it) const { return mIter2 > it.mIter2; } // second
  bool
    operator<=(const type_of_this& it) const { return mIter2 <= it.mIter2; } // second
...
public:
  ...
  binary_iterator(const F& f, const BaseIter1& it1, const BaseIter2& it2)
    : F(f), extensible_iterator(it1, it2) { }
