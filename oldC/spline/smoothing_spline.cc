// $Id: smoothing_spline.cc,v 1.17 2004/08/25 22:15:02 bob Exp $
...
{
  if ((x > mMaxX) || (x < mMinX)) 
  { std::cout << "SMSP: Warning. " << x << " outside training range.\n";
...
SmoothingSplineOperator::operator()(double x) const
{
  ...
  SmoothingSplineData *data (sDataRegistry.data(mKey));
  int nc    (data->nCoef());
  double mx (map_to_0_1(x));
  // std::cout << "\nSMOP: Calling bvalue with args " << *mKnots << " " << *mCoefs << " " << nc << " " << mx << "  order " << order << std::endl;
...
SmoothingSpline::compute_smooth(int df)
{
  ...
  */
  ...
  mLambda = dParms[3];
  std::cerr << "SMSP: Lambda set to " << mLambda << " with total leverage ";
  double totalLev (std::accumulate(lev, lev+mNUnique, 0.0)); 
  std::cerr << totalLev << std::endl;
  double ddf (df);
  if (abs((totalLev - ddf)/ddf) > 0.05)
  { std::cerr << "SMSP: *** Warning. Sum of leverages not within 5% of target df.\n";
    std::ofstream output("/Users/bob/C/spline/test/problem_xy.txt");
    if(output)
    { std::cerr << "SMSP: Dumping (x,y) data to spline/test/problem_xy.txt.\n";
      for (int i=0; i<mN; ++i)
        output << mX[i] << " " << mY[i] << std::endl;
      output.close();
    }
