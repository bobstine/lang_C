// $Id: smoothing_spline.h,v 1.17 2004/08/25 22:15:02 bob Exp $
...
class SmoothingSplineOperator : public std::unary_function<double,double>
{
 ...
 public:
  ...
  SmoothingSplineOperator (int df, double min, double max, int nCoef, double *coefs, double *knots)
    ...
    {
      // std::cerr << "SMSP: Building spline operator from input parameter list.\n";
...
class SmoothingSpline
{
 ...
 public:
  ...
  SmoothingSpline ()
    : mN(0), mX(), mY(),  mMinX(0.0), mMaxX(0.0), mRngX(0.0), mDF(0), mW(),
  ...
  SmoothingSpline (int df, Vector const& x, Vector const& y)
    : mN(x.size()), mX(0), mY(0), mMinX(0.0), mMaxX(0.0), mRngX(0.0), mDF(df), mW(0),
  ...
  template<class Iter>
    ...
    : mN(n), mX(), mY(),  mMinX(0.0), mMaxX(0.0), mRngX(0.0), mDF(df), mW(),
    mNUnique(0), mUniqueX(), mUniqueY(), mUniqueW(), mMappedX(),
    mNCoef(0), mKnots(0), mCoefs(0), mLambda(0.0), mSmooth(), mMap()
    { initialize(x,y);   }
