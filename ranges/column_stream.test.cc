...
main()   
{
  ...
  if (true)
  {
    ...
    std::vector<Scalar> v (5); v[0]=0.0; v[1]=1.0; v[2]=2.0; v[3]=3.0; v[4]=4.0;
    dict["w1"] = v;
    for(auto x : v) x += 10;
    dict["w2"] = v;
    for(auto x : v) x += 10;
    ...
    std::pair<size_t,size_t> nInserted = insert_columns_from_stream (is,dict,std::back_inserter(columns));

    std::cout << "TEST: Inserted " << nInserted.second << " columns from file having " << nInserted.first
	      << " size of column vector is " << columns.size() << " (which better match).\n";
    std::cout << "      1st is\n       " << columns[0] << std::endl;
    std::cout << "      2nd is\n       " << columns[1] << std::endl;
    std::cout << "      3rd is\n       " << columns[2] << std::endl;
