...
/*
  This version never worked due to issues related to getting the right iterator
  properties to line up.  Could not inherit from one to the next since then there
  would be ambigous definitions of terms.  Without inheritance, you lose all of the
  iterator properties from lower levels.  Messed up.

  Specializing the traits would not work due to limitations on the traits classes
  and namespaces.
*/
...
make_unary_iterator (F const& f, Iter const& iter)
{
...
make_unary_range (F const& f, Range const& r)
{
